...
int main(int argc, char *argv[])
{
    ...
    e = synccom_connect(port_num, &h);
    if (e != 0) {
        switch (e) {
        case SYNCCOM_PORT_NOT_FOUND:
    ...
    fprintf(stdout, "This is a very simple test to verify your device is\n");
...
int loop(synccom_handle h)
{
    ...
    e = synccom_write_with_blocking(h, odata, sizeof(odata), &bytes_written);
    if (e != 0) {
        fprintf(stderr, "synccom_write_with_blocking failed with %d\n", e);
        return EXIT_FAILURE;
    }

    e = synccom_read_with_timeout(h, idata, sizeof(idata), &bytes_read, 1000);
    if (e != 0) {
